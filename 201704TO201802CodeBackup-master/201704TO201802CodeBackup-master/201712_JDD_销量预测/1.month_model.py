...
t_order_sum['true_sale_amt_ratio'] = t_order_sum['rtn_amt'] / t_order_sum['sale_amt']
# 订单 - 退货
...
# 人均优惠
t_order_sum['offer_user_ration'] = t_order_sum['offer_cnt'] / t_order_sum['user_cnt']
# 金额的差值
t_order_sum['diff_amt'] = t_order_sum['sale_amt'] - t_order_sum['rtn_amt'] - t_order_sum['offer_amt']
# 指数变换
# import math
# t_order_sum['e_sale_amt'] = t_order_sum['sale_amt'] * math.e
# 实际订单消费
t_order_sum['true_sale_amt_diff'] = t_order_sum['true_sale_amt'] / t_order_sum['true_ord_cnt']
# 人均消费
t_order_sum['pre_sale'] = t_order_sum['sale_amt'] / t_order_sum['user_cnt']
...
print('bulid _4_ model')

bsts = []
bst1 = xgb.XGBRegressor(n_estimators=500,max_depth=5,seed=2017,learning_rate=0.09)
bsts.append(bst1)
bst2 = xgb.XGBRegressor(n_estimators=500,max_depth=5,seed=2017,learning_rate=0.09)
bsts.append(bst2)
bst3 = xgb.XGBRegressor(n_estimators=500,max_depth=5,seed=2017,learning_rate=0.09)
bsts.append(bst3)
bst4 = xgb.XGBRegressor(n_estimators=500,max_depth=5,seed=2017,learning_rate=0.09)
bsts.append(bst4)
...
# 0.7 0.4 0.5 0.6
res = pd.DataFrame(((res.pre0*0.9 + res.pre1*1.2 + res.pre2*0.9 + res.pre3*1)/4)).reset_index()

res['index'] = res['index'] + 1

ResFileName = './1203_xgb_4_model_result.csv'
